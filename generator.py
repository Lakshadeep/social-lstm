...
class data_augmentator():
    ...
    def __init__(self, f_prefix, num_of_data, seq_length, val_percent):
        ...
        self.num_validation_data = math.ceil(
            self.num_of_data * val_percent)  # number of validation dataset
        self.num_train_data = self.num_of_data - \
            self.num_validation_data  # number of train dataset
        print("For each dataset -----> Number of additional training dataset: ",
              self.num_train_data, " Number of validation dataset: ", self.num_validation_data)
        ...
        self.dataloader = DataLoader(
            f_prefix, 1, seq_length, 0, forcePreProcess=True, infer=False, generate=True)
    ...
    def random_dataset_creation(self):
        ...
        for batch in range(self.dataloader.num_batches):
            ...
            # Get the sequence
            x_seq, d_seq, numPedsList_seq, PedsList_seq = x[
                0], d[0], numPedsList[0], PedsList[0]

            # convert dense vector
            x_seq, lookup_seq = self.dataloader.convert_proper_array(
            ...
            if dataset_pointer_ins is not self.dataloader.dataset_pointer:
                if self.dataloader.dataset_pointer is not 0:
                    ...
                    dataset_instances = {}
                    random_angles = random.sample(
                        self.angles, self.num_of_data)  # sample new angle
                    self.noise_std = random.uniform(
                        self.noise_std_min, self.noise_std_max)  # sample new noise
                    print("Dataset creation for: ", file_name,
            ...
            for index, angle in enumerate(random_angles):
                ...
                modified_x_seq = self.submision_seq_preprocess(self.handle_seq(
                    x_seq, lookup_seq, PedsList_seq, angle), self.seq_length, lookup_seq)
                # store modified data points to dict
                self.dataloader.add_element_to_dict(
                    dataset_instances, (dir_name, file_name, index), modified_x_seq)

            end = time.time()
            print('Current file : ', file_name, ' Processed trajectory number : ', batch + 1,
                  'out of', self.dataloader.num_batches, 'trajectories in time', end - start)

        # write modified datapoints to txt files
        whole_dataset.append(dataset_instances)
        create_directories(os.path.join(
            self.f_prefix, self.base_validation_path), self.dataloader.get_all_directory_namelist())
        ...
        for ind, frame in enumerate(vectorized_x_seq):
            for ped in PedsList_seq[ind]:
                ...
                rotated_point = rotate(
                    origin, selected_point, math.radians(angle))
                ...
                # roatate first frame value as well and add it back to get
                # absoute coordinates
                modified_x_seq[ind, lookup_seq[ped], 0] = (rotate(origin, first_values_dict[
                                                           ped], math.radians(angle)))[0] + modified_x_seq[ind, lookup_seq[ped], 0]
                modified_x_seq[ind, lookup_seq[ped], 1] = (rotate(origin, first_values_dict[
                                                           ped], math.radians(angle)))[1] + modified_x_seq[ind, lookup_seq[ped], 1]
    ...
    def submision_seq_preprocess(self, x_seq, seq_lenght, lookup_seq):
        ...
        id_integrated_seq = np.append(
            np.array(ped_ids)[:, None], ret_x_seq_c_selected, axis=1)
        frame_integrated_seq = np.append(
            frame_numbers[:, None], id_integrated_seq, axis=1)
    ...
    def write_modified_datasets(self, dataset_instances_store):
        ...
        for dataset_index in range(self.dataloader.numDatasets):
            ...
            train_sub_instances = dict(random.sample(
                dataset_instances.items(), self.num_train_data))
            validation_sub_instances = {
                k: v for k, v in dataset_instances.items() if k not in train_sub_instances}
            print(
                "*********************************************************************************")
            print("Training datasets are writing for: ",
                  self.dataloader.get_file_name(dataset_index))
            self.write_dict(train_sub_instances, self.base_train_path)
            print(
                "*********************************************************************************")
            print("Validation datasets are writing for: ",
                  self.dataloader.get_file_name(dataset_index))
            self.write_dict(validation_sub_instances,
    ...
    def clear_directories(self, base_path, delete_all=False):
        ...
        for dir_ in dir_names:
            ...
            [delete_file(dir_path, [file_name])
             for file_name in file_names if file_name not in base_file_names]
...
def main():
    ...
    print("Percentage of data will be allocated for validation: %",
          args.validation * 100)
    ...
    augmentator = data_augmentator(
        f_prefix, args.num_data, args.seq_length, args.validation)
