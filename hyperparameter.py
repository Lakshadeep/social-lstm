...
def main():
    ...
    dataloader_t = DataLoader(f_prefix, args.batch_size, args.seq_length,
                              num_of_validation=args.num_validation, forcePreProcess=True, infer=True)
    dataloader_v = DataLoader(f_prefix, 1, args.seq_length,
                              num_of_validation=args.num_validation, forcePreProcess=True, infer=True)
    ...
    for hyperparams in itertools.islice(sample_hyperparameters(), args.num_samples):
            ...
            for batch in range(dataloader_t.num_batches):
                ...
                for sequence in range(dataloader_t.batch_size):
                    ...
                    if args.method == 3:  # vanilla lstm
                        outputs, _, _ = net(
                            x_seq, hidden_states, cell_states, PedsList_seq, numPedsList_seq, dataloader_t, lookup_seq)

                    else:
                        outputs, _, _ = net(x_seq, grid_seq, hidden_states, cell_states,
                                            PedsList_seq, numPedsList_seq, dataloader_t, lookup_seq)
        ...
        for batch in range(dataloader_v.num_batches):
            ...
            if dataset_pointer_ins is not dataloader_v.dataset_pointer:
                if dataloader_v.dataset_pointer is not 0:
                    print('Finished prosessed file : ', dataloader_v.get_file_name(-1),
            ...
            for sequence in range(dataloader_v.batch_size):
                ...
                if args.method == 3:  # vanilla lstm
                    ret_x_seq, loss = sample_validation_data_vanilla(
                        x_seq, PedsList_seq, args, net, lookup_seq, numPedsList_seq, dataloader_v)

                else:
                    ret_x_seq, loss = sample_validation_data(
                        x_seq, PedsList_seq, grid_seq, args, net, lookup_seq, numPedsList_seq, dataloader_v)
                ...
                err = get_mean_error(ret_x_seq.data, orig_x_seq.data,
                                     PedsList_seq, PedsList_seq, args.use_cuda, lookup_seq)
            ...
            print('Current file : ', dataloader_v.get_file_name(0), ' Batch : ', batch + 1, ' Sequence: ',
        ...
        if dataloader_v.num_batches != 0:
            ...
            print('valid_loss = {:.3f}, valid_mean_err = {:.3f}, valid_final_err = {:.3f}, score = {:.3f}, time = {:.3f}'.format(
                loss_epoch, err_epoch, f_err_epoch, avg_err, elapsed_time))
            log_file.write('valid_loss = {:.3f}, valid_mean_err = {:.3f}, valid_final_err = {:.3f}, score = {:.3f}, time = {:.3f}'.format(
                loss_epoch, err_epoch, f_err_epoch, avg_err, elapsed_time) + '\n')

    print("--------------------------Best ", args.best_n,
          " configuration------------------------")
    log_file.write("-----------------------------Best " +
                   str(args.best_n) + " configuration---------------------" + '\n')
