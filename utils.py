...
class DataLoader():
    ...
    def __init__(self, f_prefix, batch_size=5, seq_length=20, num_of_validation=0, forcePreProcess=False, infer=False, generate=False):
        ...
        self.dataset_dimensions = {'biwi': [720, 576], 'crowds': [
            720, 576], 'stanford': [595, 326], 'mot': [768, 576]}
        ...
        self.train_dataset = self.get_dataset_path(
            self.base_train_path, f_prefix)
        self.test_dataset = self.get_dataset_path(
            self.base_test_path, f_prefix)
        self.validation_dataset = self.get_dataset_path(
            self.base_validation_path, f_prefix)

        # if generate mode, use directly train base files
        if generate:
            self.train_dataset = [os.path.join(
                f_prefix, dataset[1:]) for dataset in base_train_dataset]
        ...
        # check validation dataset availibility and clip the reuqested number
        # if it is bigger than available validation dataset
        if self.additional_validation:
            if len(self.validation_dataset) is 0:
                print("There is no validation dataset.Aborted.")
                self.additional_validation = False
            else:
                num_of_validation = np.clip(
                    num_of_validation, 0, len(self.validation_dataset))
                self.validation_dataset = random.sample(
                    self.validation_dataset, num_of_validation)
        ...
        self.data_file_tr = os.path.join(
            self.train_data_dir, "trajectories_train.cpkl")
        self.data_file_te = os.path.join(
            self.base_test_path, "trajectories_test.cpkl")
        self.data_file_vl = os.path.join(
            self.val_data_dir, "trajectories_val.cpkl")
        ...
        if self.additional_validation:
            ...
            if not(os.path.exists(self.data_file_vl)) or forcePreProcess:
                ...
                self.frame_preprocess(
                    self.validation_dataset, self.data_file_vl, self.additional_validation)
        ...
        if self.infer:
            ...
            # if infer mode, and there are additional validation files ->
        ...
        # if not infer mode
            ...
            # If the file doesn't exist or forcePreProcess is true -> training
            # pre-process
    ...
    def frame_preprocess(self, data_dirs, data_file, validation_set=False):
        ...
        # Ech list would contain the number of pedestrians in each frame in the
        # dataset
        numPeds_data = []
        ...
        for directory in data_dirs:
            ...
            if self.infer is False:
                df = pd.read_csv(directory, dtype={
                                 'frame_num': 'int', 'ped_id': 'int'}, delimiter=' ',  header=None, names=column_names)
                self.target_ids = np.array(df.drop_duplicates(
                    subset={'ped_id'}, keep='first', inplace=False)['ped_id'])

            else:
                # if validation mode, read validation file to pandas dataframe
                # and process
                if self.additional_validation:
                    df = pd.read_csv(directory, dtype={
                                     'frame_num': 'int', 'ped_id': 'int'}, delimiter=' ',  header=None, names=column_names)
                ...
                # if test mode, read test file to pandas dataframe and process
                    ...
                    column_names = ['frame_num', 'ped_id', 'y', 'x']
                    df = pd.read_csv(directory, dtype={'frame_num': 'int', 'ped_id': 'int'}, delimiter=' ',  header=None, names=column_names, converters={
                                     c: lambda x: float('nan') if x == '?' else float(x) for c in ['y', 'x']})
                    self.target_ids = np.array(df[df['y'].isnull()].drop_duplicates(
                        subset={'ped_id'}, keep='first', inplace=False)['ped_id'])
            ...
            for ind, frame in enumerate(frameList):
                ...
                # At inference time, data generation and if dataset is a
                # validation dataset, no validation data
                if (ind >= numFrames * self.val_fraction) or (self.infer) or (self.generate) or (validation_set):
                    # Add the details of all the peds in the current frame to
                    # all_frame_data
                    ...
                    valid_frame_data[dataset_index].append(
        ...
        pickle.dump((all_frame_data, frameList_data, numPeds_data, valid_numPeds_data,
                     valid_frame_data, pedsList_data, valid_pedsList_data, target_ids, orig_data), f, protocol=2)
        f.close()
    ...
    def load_preprocessed(self, data_file, validation_set=False):
        ...
        for dataset in range(len(self.data)):
            ...
            num_valid_seq_in_dataset = int(
                len(valid_frame_data) / (self.seq_length))
            if not validation_set:
                print('Training data from training dataset(name, # frame, #sequence)--> ',
                      dataset_name, ':', len(all_frame_data), ':', (num_seq_in_dataset))
                print('Validation data from training dataset(name, # frame, #sequence)--> ',
                      dataset_name, ':', len(valid_frame_data), ':', (num_valid_seq_in_dataset))
            else:
                print('Validation data from validation dataset(name, # frame, #sequence)--> ',
                      dataset_name, ':', len(all_frame_data), ':', (num_seq_in_dataset))

            # Increment the counter with the number of sequences in the current
            # dataset
    ...
    def next_batch(self):
        ...
        while i < self.batch_size:
            ...
            # While there is still seq_length number of frames left in the
            # current dataset
            if idx + self.seq_length - 1 < len(frame_data):
                ...
                target_ids.append(self.target_ids[self.dataset_pointer][
                                  math.floor((self.frame_pointer) / self.seq_length)])
                ...
                # Increment the dataset pointer and set the frame_pointer to
                # zero
    ...
    def next_valid_batch(self):
        ...
        while i < self.batch_size:
            ...
            if idx + self.seq_length < len(frame_data):
                ...
                # get correct target ped id for the sequence
                target_ids.append(self.target_ids[self.dataset_pointer][
                                  math.floor((self.valid_frame_pointer) / self.seq_length)])
    ...
    def tick_batch_pointer(self, valid=False):
        ...
        if not valid:
            ...
            print("now processing: %s" %
                  self.get_file_name(pointer_type='valid'))
    ...
    def switch_to_dataset_type(self, train=False, load_data=True):
        # function for switching between train and validation datasets during
        # training session
        print('--------------------------------------------------------------------------')
    ...
    def convert_proper_array(self, x_seq, num_pedlist, pedlist):
        ...
        # output: seq_lenght (real sequence lenght+1)*max_ped_id+1 (biggest id
        # number in the sequence)*2 (x,y)

        # get unique ids from sequence
        unique_ids = pd.unique(np.concatenate(
            pedlist).ravel().tolist()).astype(int)
    ...
    def get_directory_name(self, offset=0):
        # return folder name of file of processing or pointing by dataset
        # pointer
        folder_name = self.data_dirs[
            self.dataset_pointer + offset].split('/')[-2]
        return folder_name
    ...
    def get_all_directory_namelist(self):
        ...
        folder_list = [data_dir.split('/')[-2]
                       for data_dir in (self.base_data_dirs)]
        return folder_list
    ...
    def clean_test_data(self, x_seq, target_id, obs_lenght, predicted_lenght):
        # remove (pedid, x , y) array if x or y is nan for each frame in
        # observed part (for test mode)
        for frame_num in range(obs_lenght):
            nan_elements_index = np.where(np.isnan(x_seq[frame_num][:, 2]))

            try:
                x_seq[frame_num] = np.delete(
                    x_seq[frame_num], nan_elements_index[0], axis=0)
            except ValueError:
    ...
    def write_dataset(self, dataset_seq, file_name, path):
        ...
        np.savetxt(os.path.join(path, file_name), out,
                   fmt="%1d %1.1f %.3f %.3f", newline='\n')
    ...
    def write_to_plot_file(self, data, path):
        ...
        for file in range(self.numDatasets):
            ...
            print("Writing to plot file  path: %s, file_name: %s" %
                  (path, file_name))
    ...
    def get_frame_sequence(self, frame_lenght):
        ...
        frame_number = self.orig_data[self.dataset_pointer][
            begin_fr:end_fr, 0].transpose()
        return frame_number
    ...
    def get_id_sequence(self, frame_lenght):
        ...
        begin_fr = (self.frame_pointer - frame_lenght)
        end_fr = (self.frame_pointer)
        id_number = self.orig_data[self.dataset_pointer][
            begin_fr:end_fr, 1].transpose()
