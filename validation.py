...
def main():
    ...
    dataloader = DataLoader(f_prefix, 1, sample_args.seq_length,
                            num_of_validation=sample_args.num_validation, forcePreProcess=True, infer=True)
        ...
        for sequence in range(dataloader.batch_size):
            ...
            if sample_args.method == 3:  # vanilla lstm
                ret_x_seq, loss = sample_validation_data_vanilla(
                    x_seq, PedsList_seq, sample_args, net, lookup_seq, numPedsList_seq, dataloader)

            else:
                ret_x_seq, loss = sample_validation_data(
                    x_seq, PedsList_seq, grid_seq, sample_args, net, lookup_seq, numPedsList_seq, dataloader)
            ...
            err = get_mean_error(ret_x_seq.data, orig_x_seq.data, PedsList_seq,
                                 PedsList_seq, sample_args.use_cuda, lookup_seq)
            ...
            ), PedsList_seq, lookup_seq, dataloader.get_frame_sequence(sample_args.seq_length), target_id))
    ...
    if dataloader.num_batches != 0:
        ...
        print('valid_loss = {:.3f}, valid_mean_err = {:.3f}, valid_final_err = {:.3f}'.format(
            loss_epoch, err_epoch, f_err_epoch))

    dataloader.write_to_plot_file(epoch_result, os.path.join(
