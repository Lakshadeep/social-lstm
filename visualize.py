...
def save_video(sequence_path, video_path, video_name, frame):
    ...
    command = "ffmpeg -r " + \
        str(frame) + " -i " + image_input_path + \
        " -vcodec mpeg4 -y " + video_output_path
    print("Creating video: input sequence --> ", image_input_path,
...
def annotate_plot(plt, x_vals, y_vals, annotation):
    ...
    for index, (i, j) in enumerate(zip(x_vals, y_vals)):
        texts.append(plt.text(i, j, annotation[
...
def get_min_max_in_trajectories(traj):
    ...
    max_val_x = np.max(([np.nanmax(val_list)
                         for val_list in concat_list_vals]))
    min_val_x = np.min(([np.nanmin(val_list)
                         for val_list in concat_list_vals]))
    concat_list_vals = [np.concatenate(seq[:, :, 1]) for seq in traj]
    max_val_y = np.max(([np.nanmax(val_list)
                         for val_list in concat_list_vals]))
    min_val_y = np.min(([np.nanmin(val_list)
                         for val_list in concat_list_vals]))
...
def plot_trajectories(true_trajs, pred_trajs, nodesPresent, look_up, frames, name, plot_directory, min_threshold, max_ped_ratio, target_id, style, obs_length=20):
    ...
    # frame will be taken for rotation vector (starting point (0,0), ending
    # point: this frame)
    target_frame = 1
    video_plot_trajs = []
    '''
    ...
    # translate all trajectories using first frame of target ped to center
    # target ped traj
    true_trajs = translate(torch.from_numpy(true_trajs), nodesPresent,
                           look_up, torch.from_numpy(true_trajs[0][look_up[target_id], 0:2]))
    pred_trajs = translate(torch.from_numpy(pred_trajs), nodesPresent,
                           look_up, torch.from_numpy(pred_trajs[0][look_up[target_id], 0:2]))
    ...
    dict_pred = get_min_max_in_sequence(pred_trajs[:, look_up[target_id], :][
                                        :, None, :].numpy())  # only consider prediciton of target id because of model
    # print(dict_pred)
    # print(dict_true)
    overaall_max_x, overaall_min_x, overaall_max_y, overaall_min_y = get_overall_max_min(
        dict_true, dict_pred)
    ...
    # inverse lookup table -> array indices : ped_ids
    ...
    # maximum number of peds will be processed
    ...
    # add target id to at the beginnnig of the list therefore it will process
    # always
    ...
    for j in shuffled_ped_ids:
        ...
        # assign processed ped number and ped id
        real_inv_lookup[processed_ped_number] = ped_id

        # skip the trajectory if len is smaller than 2 or min_threshold
        ...
        # exctract non-nan values and scale it according to min and max overall
        # values in this frame
        filtered_true_x = min_max_scaler(np.array([x for x in true_x if not np.isnan(
            x)]), overaall_min_x, overaall_max_x, min_r, max_r)
        filtered_true_y = min_max_scaler(np.array([y for y in true_y if not np.isnan(
            y)]), overaall_min_y, overaall_max_y, min_r, max_r)
        filtered_pred_x = min_max_scaler(np.array([x for x in pred_x if not np.isnan(
            x)]), overaall_min_x, overaall_max_x, min_r, max_r)
        filtered_pred_y = min_max_scaler(np.array([y for y in pred_y if not np.isnan(
            y)]), overaall_min_y, overaall_max_y, min_r, max_r)
        ...
        if ped_id == target_id:  # if this is target ped, plot predicted line as well
            ...
            pred_line = plt.plot(filtered_pred_x, filtered_pred_y, linestyle=next(
                lines), marker=next(marker), linewidth=line_width, label=pred_ped_text)
        ...
        true_line = plt.plot(filtered_true_x, filtered_true_y, linestyle=next(
            lines), marker=next(marker), linewidth=line_width, label=true_ped_text)
    ...
    plt.gca().set_xlim([min_r - true_target_id_values[0] -
                        plot_offset, max_r - true_target_id_values[0] + plot_offset])
    plt.gca().set_ylim([min_r - true_target_id_values[1] -
                        plot_offset, max_r - true_target_id_values[1] + plot_offset])

    # save figure
    plt.savefig(plot_directory + '/' + name + '.png')
    plt.gcf().clear()
    # video animation
    create_plot_animation(plt, video_plot_trajs, processed_ped_index, target_id, real_inv_lookup, obs_length,
                          markers, colors, name, frames, true_target_id_values, plot_directory, style, num_of_color)
    plt.close()
...
def create_plot_animation(plt, trajs, shuffled_ped_ids, target_id, inv_lookup, seq_lenght, marker, colors, name, data_frames, true_target_id_values, plot_directory, style, num_of_color):
    ...
    for frame_num in range(0, seq_lenght - 1):
        ...
        for elem_index, index in enumerate(shuffled_ped_ids):
            ...
            marker_cycle = get_marker_style(
                marker, marker_cycle, elem_index, num_of_color)
            if frame_num == 0:
            ...
            if ped_id == target_id:
                ...
                pred_ped_text = 'target ped ' + str(ped_id) + ' pred.'
                if frame_num == 0:
                    pred_sc = plt.scatter(filtered_pred_x[0:real_pred_index + 1], filtered_pred_y[
                                          0:real_pred_index + 1], marker=marker_arr[elem_index], label=pred_ped_text)

                else:
                    pred_sc = plt.scatter(filtered_pred_x[0:real_pred_index + 1], filtered_pred_y[
                                          0:real_pred_index + 1], marker=marker_arr[elem_index])

                frame_obj.append(pred_sc)

            if frame_num == 0:
                true_sc = plt.scatter(filtered_true_x[0:real_true_index + 1], filtered_true_y[
                                      0:real_true_index + 1], marker=marker_arr[elem_index], label=true_ped_text)

            else:
                true_sc = plt.scatter(filtered_true_x[0:real_true_index + 1], filtered_true_y[
                                      0:real_true_index + 1], marker=marker_arr[elem_index])
    ...
    ani = animation.ArtistAnimation(
        plt.gcf(), frames, interval=1200, blit=False, repeat_delay=1000)
...
def vectorize_traj(traj, nodesPresent, look_up):
    ...
    traj, _ = vectorize_seq(
        Variable(torch.FloatTensor(traj)), nodesPresent, look_up)
...
def rotate_traj(traj_x, traj_y, angle):
    ...
    for p_index in range(len(traj_x)):
        rotated_points = rotate(
            origin, (traj_x[p_index], traj_y[p_index]), angle)
...
def get_final_traj_error(true_trajs, pred_trajs):
    ...
    print("true traj: ", last_true_point, "pred_traj: ",
          last_pred_point, "error: ", error)

    return error


def plot_target_trajs(trajs, plot_directory, num_of_traj, plot_offset):
    # method will be plot all target ped trajectories for each sequence in
    # final
    line_width = 3
    num_of_color = 20
    fig_width = 10
    ...
    # calculate error for each trajectory and return error values and their
    ...
    for arr_index, index in enumerate(biggest_err_index):
        ...
        marker_cycle = get_marker_style(
            marker, marker_cycle, arr_index * 2, num_of_color)
        line_cycle = get_marker_style(
            line_style, line_cycle, arr_index * 2, num_of_color)
        ...
        overall_max_x = max(np.max(true_traj[0]), np.max(
            pred_traj[0]), overall_max_x)
        overall_min_x = min(np.min(true_traj[0]), np.min(
            pred_traj[0]), overall_min_x)
        overall_max_y = max(np.max(true_traj[1]), np.max(
            pred_traj[1]), overall_max_y)
        overall_min_y = min(np.min(true_traj[1]), np.min(
            pred_traj[1]), overall_min_y)
        ...
        print("**************************")
        true_line = plt.plot(true_traj[0], true_traj[1], linestyle=next(
            line_cycle), marker=next(marker_cycle), linewidth=line_width, label=true_txt)
        pred_line = plt.plot(pred_traj[0], pred_traj[1], linestyle=next(
            line_cycle), marker=next(marker_cycle), linewidth=line_width, label=pred_txt)

    # plot adjustmment
    plt.gca().set_xlim(
        [overall_min_x - plot_offset, overall_max_x + plot_offset])
    plt.gca().set_ylim(
        [overall_min_y - plot_offset, overall_max_y + plot_offset])
...
def main():
    ...
    # creation of paths
    save_plot_directory = os.path.join(
        f_prefix, 'plot', method_name, model_name, 'plots/')
    plot_directory = os.path.join(
        f_prefix, 'plot', method_name, model_name, plot_file_directory)
    video_directory = os.path.join(
        f_prefix, 'plot', method_name, model_name, 'videos/')
    ...
    for file_index in range(len(plot_file_name)):
        ...
        for i in range(len(results)):
            ...
            if args.num_of_data is 0:  # test data visualization
                target_traj = plot_trajectories(results[i][0], results[i][1], results[i][2], results[i][3], results[i][
                                                4], name, figure_save_directory,  args.min_traj, args.max_ped_ratio, results[i][5], [min_r, max_r, plot_offset], results[i][6])
            else:
                target_traj = plot_trajectories(results[i][0], results[i][1], results[i][2], results[i][3], results[i][
                                                4], name, figure_save_directory, args.min_traj, args.max_ped_ratio, results[i][5], [min_r, max_r, plot_offset], 20)
            target_traj.append(results[i][2])  # pedlist
            target_traj.append(results[i][3])  # lookup
            target_id_trajs.append(target_traj)

        save_video(figure_save_directory, video_save_directory,
                   plot_file_name[file_index], args.frame)
        plot_target_trajs(target_id_trajs, figure_save_directory,
                          args.max_target_ped, plot_offset)
